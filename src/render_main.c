...
static void		draw_local_wall(t_img *img, t_wall *wall, t_renderer *r, int x)
{
	int ya;
	int yb;
	...
	draw_line(img, &(t_vline){ x, r->top_limit[x],
									wall->cya, 0, 0x003399, 0 });
	draw_line(img, &(t_vline){ x, wall->cyb,
							r->bottom_limit[x], 0, 0x009933, 0 });
	if (wall->neighbor < 0)
		draw_line(img, &(t_vline){ x, wall->cya,
			wall->cyb, 0, x == wall->x1 || x == wall->x2 ? 0 : 0x444444, 0 });
}

static void		draw_neighbor_wall(t_img *img, t_wall *wall,
												t_renderer *r, int x)
{
	int nya;
	int nyb;
	...
	draw_line(img, &(t_vline){ x, wall->cya, wall->ncya - 1, 0, 0x440077, 0 });
	r->top_limit[x] = clampf(maxf(wall->cya, wall->ncya),
												r->top_limit[x], img->h - 1);
	draw_line(img, &(t_vline){ x, wall->ncyb + 1, wall->cyb,
						0, x == wall->x1 || x == wall->x2 ? 0 : 0x770044, 0 });
	r->bottom_limit[x] = clampf(minf(wall->cyb, wall->ncyb),
														0, r->bottom_limit[x]);
}

void			render_wall(t_img *img, t_renderer *renderer, t_wall *wall,
											t_render_item const *current_sector)
{
	...
	while (++x <= endx)
	{
		draw_local_wall(img, wall, renderer, x);
		if (wall->neighbor >= 0)
			draw_neighbor_wall(img, wall, renderer, x);
	}
...
void			draw_screen(t_img *img, t_map *map)
{
	...
	init_renderer(&r, img, map->number_sectors);
	(*r.head) = (t_render_item){map->player.sector_number, 0, img->w - 1};
	++r.head;
