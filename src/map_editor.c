	...
	if (e->mode == TEXTURE && e->sectors[e->n].num_walls < WALLS_CNT - 1)
	{
		...
		else
		{
			if (e->sectors[e->n].num_walls > 0)
				e->sectors[e->n].walls[e->sectors[e->n].num_walls].begin =
				e->sectors[e->n].walls[e->sectors[e->n].num_walls - 1].end;
...
void				create_sector(t_map_editor *e)
{
	...
	e->sectors[e->n].walls[0].begin =
		e->sectors[e->n - 1].walls[e->chosen].end;
	e->sectors[e->n].walls[0].end =
		e->sectors[e->n - 1].walls[e->chosen].begin;
	e->sectors[e->n].walls[1].global_index =
		e->sectors[e->n - 1].walls[e->chosen].global_index;
	if (e->sectors[e->n - 1].walls[e->chosen].global_index !=
		e->sectors[e->n - 1].num_walls - 1)
	...
	if (m->sdl.e.type == SDL_KEYDOWN || m->sdl.e.type == SDL_MOUSEBUTTONDOWN)
	{
			...
			(e->n < SECTORS_CNT && m->sdl.e.key.keysym.sym == SDLK_RETURN))
		{
			if ((e->sectors[e->n].num_walls > 0 && e->mode == TEXTURE
				...
				(e->mode)++;
			if ((e->mode > CLOSE && e->mode < SPRITE_Z
				&& e->selected_row < TEXT_MENU / 2))
				(e->selected_row)++;
